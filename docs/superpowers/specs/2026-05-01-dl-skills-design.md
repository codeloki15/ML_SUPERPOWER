...
| `dl-prior-art` may be too narrow for novel / cross-domain tasks | The skill stays within Kaggle + HF cookbook sources and returns ONE recommended playbook per call. It does not synthesize across domains, mix-and-match winning strategies from multiple sources, or mine the long tail of failure modes. Mitigation: watch real v0.2.0 usage on 5-10 DL tasks. If `dl-prior-art` consistently misses (cross-domain fusion, strategy comparison, sparse domains where Kaggle has no match), v0.3.0 candidate is a NARROW companion skill — `dl-strategy-compose` (combine 2-3 winning playbooks into a hybrid) or `dl-failure-mining` (mine bottom-half solutions for pitfalls). Sharp anti-trigger required to avoid stealing invocations from `dl-prior-art` and `ml-engineer-research`. Do NOT add a generic "strategy maker" skill — over-fires and creates routing ambiguity. |
